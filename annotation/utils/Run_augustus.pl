...
for my $inputfa (@inputfa){
    ...
    while (my $seq=$fa->next_seq){
        ...
        my $seq=$seq->seq;
        my $len=length($seq);
        my $outputdir2="$outputdir/temp/$id";
	`mkdir -p $outputdir2` if (! -e "$outputdir2");
	print SH2 "$perlbase/Merge_augustus.pl $outputdir2 $outputdir/results/$id.gff\n";
	if ($len >= $max){
	    for (my $i=0;$i<$len;$i=$i+$window-$step){
                ...
                print O ">$id\n$newseq\n";
                close O;
		print SH "$software --species=$db $outputdir2/$id.$s-$e.fa | perl $perlbase/ConvertFormat_augustus.pl - $outputdir2/$id.$s-$e.fa.gff\n";
	    }
	}else{
	    print SH "$software --species=$db $inputfa  | perl $perlbase/ConvertFormat_augustus.pl - $outputdir2/$id.0-$len.fa.gff\n";
	}
    }
}
close SH;
