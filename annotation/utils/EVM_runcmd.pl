...
for my $chrfa (@chrfa){
    ...
    my $chr=$1;
    print  "cd $rundir/evm_for_each_chr/$chr ; perl $base/EVM_Partition_Combin.pl $evm_dir $genome_dir/$chr.fa ab_initio.gff homolog.gff rna_seq.gff 1000000 100000 $weights > split_evm_running.sh ; cd $rundir\n";
}
