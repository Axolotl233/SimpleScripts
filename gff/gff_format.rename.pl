...
while(<IN>){
    ...
    }elsif($line[2] =~ /five_prime_UTR/){
	$line[8] = "ID=$new$num.five_prime_UTR;Parent=$new$num.t1";
    }elsif($line[2] =~ /three_prime_UTR/){
	$line[8] = "ID=$new$num.three_prime_UTR;Parent=$new$num.t1";
    }
    $p = join"\t",@line;
    print $p."\n";
}
