...
yTickMarks <- cumsum(as.numeric(chromMax_query))
xTickMarks <- cumsum(as.numeric(chromMax_ref))

gp <- ggplot(alignments) +
  #geom_point(mapping = aes(x = refStart2, y = queryStart2, color = percentID),size = 0.0005)+ 
  #geom_point(mapping = aes(x = refEnd2, y = queryEnd2, color = percentID),size = 0.0005)+
  geom_segment(
    aes(
      x = refStart2,
      xend = refEnd2,
  ...
  scale_x_continuous(
    ...
    labels = c("",names(chromMax_ref))
    )+
  theme_bw()+
  theme(
    panel.grid.minor.y = element_blank(),
    panel.grid.minor.x = element_blank(),
    panel.grid.major = element_line(linewidth = 0.008,linetype = "dashed"),
    axis.text.x = element_text(angle = 90,vjust = 0.5,hjust =1)
    )+
  scale_y_continuous(
    breaks = c(0,yTickMarks), 
    labels = c("",names(chromMax_query))
    )+
