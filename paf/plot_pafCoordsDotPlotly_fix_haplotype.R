...
## Modified by Wenjie Mu, Lanzhou university
...
option_list <- list(
  ...
  make_option(c("-q", "--min-query-length"), type="numeric", default=100000,
...
alignments$queryGr <- gsub("tg.*",replacement = "",alignments$queryID)
# Fixes for PAF
...
if (opt$similarity) {
  gp = ggplot(alignments) +
    geom_point(
      mapping = aes(x = refStart2, y = queryStart2, color = queryGr),
      size = 0.009
    ) +
    geom_point(
      mapping = aes(x = refEnd2, y = queryEnd2, color = queryGr),
      size = 0.009
    ) +
    geom_segment(
      aes(
...
# gp
t = opt$plot_size-1.5
ggsave(filename = paste0(opt$output_filename, ".pdf"), width = opt$plot_size, height = t)
